...
Uses kepler_processed.csv (20 features + target)
...
print("="*80)
print("KEPLER - TRAINING 3 CLASSIFIERS")
print("="*80)
...
print("\n[1/6] Loading kepler_processed.csv...")
...
fig.suptitle('KEPLER - Model Comparison (XGBoost, Random Forest, SVM)',
...
colors = ['green' if auc > 0.9 else 'orange' if auc > 0.8 else 'red' for auc in aucs]
...
plt.tight_layout()
plt.savefig('kepler_model_comparison.png', dpi=300, bbox_inches='tight')
print("   -> kepler_model_comparison.png")
...
comparison_df.to_csv('kepler_models_comparison.csv', index=False)
